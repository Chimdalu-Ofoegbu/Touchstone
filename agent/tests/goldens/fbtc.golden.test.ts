...
// [2-04-03 FBTC] End-to-end pipeline golden test (mirrors usdy.golden.test.ts).
...
function makeClient(letter: GradeLetter, uint8: number) {
  const args = {
    ...
    subject: {
      name: "FBTC",
      ticker: "FBTC" as const,
      address: "0xC96dE26018A54D51c097160568752c4E3BD6C364" as `0x${string}`,
    ...
    overall_rationale: "Overall solid across all four dimensions for FBTC.",
  };
...
describe("[2-04-03 FBTC] golden ReasoningDocument", () => {
  beforeEach(() => {
  ...
  it("produces a schema-valid ReasoningDocument with hash format 0x+64 hex", async () => {
    ...
    expect(doc.subject.ticker).toBe("FBTC");
