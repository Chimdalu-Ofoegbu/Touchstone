...
- `pinata` SDK wiring details (raw-file upload call, CID version), agent-card JSON contents.
...
- **IPFS provider — RESOLVED 2026-06-10: Pinata (amends DEC-ipfs-provider-web3storage).** Phase 3 ships **Pinata** (`pinata` SDK, single `PINATA_JWT`). The web3.storage/Storacha headless path was confirmed viable but is fiddlier (delegation: key+space+proof → `STORACHA_KEY`+`STORACHA_PROOF`) and its `uploadFile` always directory-wraps. Pinata gives a **raw-file CID** that resolves straight to the JSON — cleaner for the on-chain `cid` + Phase-4 re-hash. See the PROJECT.md `DEC-ipfs-provider-web3storage` amendment + 03-RESEARCH.md Open Questions (RESOLVED) #1. Implementation: `ipfs.ts` exposes `pin(canonicalBytes)→cid`; pin RAW (not directory-wrapped); store bare CID; identical canonical serialization on pin + verify; prove a standalone pin→fetch-via-public-gateway→re-hash→assert round trip BEFORE wiring into the pipeline. No Storacha adapter is built (single provider).
...
2. ⬜ **`PINATA_JWT` env var** — user creates a Pinata account → API Keys → New Key (pinning scope) → copies the JWT into the root `.env` (alongside `ANTHROPIC_API_KEY` / `MANTLE_RPC_URL` / `PRIVATE_KEY`). Optionally `PINATA_GATEWAY` for a dedicated gateway. Needed before the watcher/publish pipeline can pin. (Provider locked to Pinata 2026-06-10 — see amendment above.)
</decisions>
