...
- **D-11 (Model):** `claude-sonnet-4-5`. Strong reasoning + cited rationale quality + ~3-5s typical latency. Configurable via `CLAUDE_MODEL` env var so a runtime swap to `claude-opus-4-7` is one variable change if rationale quality needs to be turned up at demo time. The default in code MUST be `claude-sonnet-4-5`.
...
type ReasoningDocument = {
  ...
  claude_model: string; // e.g., "claude-sonnet-4-5"
